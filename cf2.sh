...
class ${UPPER}Screen extends StatefulWidget {
  const ${UPPER}Screen({super.key});

  @override
  State<${UPPER}Screen> createState() => _${UPPER}ScreenState();
}

class _${UPPER}ScreenState extends State<${UPPER}Screen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(child: Text('This is ${UPPER} screen')),
    );
  }
}
EOF
# Add export to feature.dart
FEATURE_FILE="lib/features/feature.dart"
MODULE_EXPORT="export '$NAME/$NAME.dart';"

# Create file if not exists
if [ ! -f "$FEATURE_FILE" ]; then
  touch "$FEATURE_FILE"
  echo "// Auto-generated feature exports" > "$FEATURE_FILE"
fi

# Check if export already exists
if ! grep -Fxq "$MODULE_EXPORT" "$FEATURE_FILE"; then
  echo "$MODULE_EXPORT" >> "$FEATURE_FILE"
  echo "Added export to feature.dart"
else
  echo "Export already exists in feature.dart"
fi
